...
$("#estName").autocomplete({
    ...
    select: function (event, ui) {
        if (ui.item.id == '') {
            ...
            $('#estAddress').val(ui.item.event_location);
            // $('#estCity').val(ui.item.organizer_city);
            // $('#estState').val(ui.item.organizer_state);
            // $('#estZip').val(ui.item.organizer_zip);
            // $('#estPhone').val(ui.item.event_phone);
