...
$("#micro_market_name" ).autocomplete({
        ...
        $.ajax({
            ...
            success: function( data ) {
                ...
                if(data.length>0)
                {
                    response($.map(data, function (item) {
                         return {
                            ...
                            micro_market_address : item.micro_market_address,
                            est_city : item.est_city,
                            est_state : item.est_state,
                            est_zip : item.est_zip,
                            micro_market_phone : item.micro_market_phone,
                ...
                else
                {
                    ...
                    $('#permit_number').val('');
                    $('#est_name').val('');
                    $('#micro_market_address').val('');
                    $('#est_city').val('');
                    $('#est_state').val('');
                    $('#est_zip').val('');
                    $('#micro_market_phone').val('');
                    $('#owner_id').val('');
                    $('#contact').val('');
                    $('#owner_name').val('');
                    $('#ownerAddress').val('');
    ...
    select: function (event, ui) {
        if(ui.item.id == ''){
            ...
            $('#micro_market_address').val(ui.item.micro_market_address);
            $('#est_city').val(ui.item.est_city);
            $('#est_state').val(ui.item.est_state);
            $('#est_zip').val(ui.item.est_zip);
            $('#micro_market_phone').val(ui.item.micro_market_phone);
...
function makeTabActiveById(id, currentPage)
{
    if(id == 2){
        ...
        $('#micro_market_name').removeClass('border-danger');
        if(permitId == ''){
            Swal.fire({
                icon: 'info',
                title: 'Please Selest the Establishment Name'
            });
            $('#micro_market_name').addClass(' border-danger');
            return false;
        }
        ...
        if(id > currentPage){
            ...
            formData.append('micro_market_name', $('#micro_market_name').val());
            formData.append('owner_name', $('#owner_name').val());
            formData.append('micro_market_address', $('#micro_market_address').val());
            formData.append('micro_market_phone', $('#micro_market_phone').val());
...
function saveNewFoodTemp(inspection_id)
{
    ...
    $.ajax({
        ...
        url: micro_market_temperature_submit_route,
        type: "POST",
        data: {
