...
$(document).ready(function () {
    ...
    $('button#btnInspectionAddMore').on({
        ...
        "click": function () {
            ...
            newInspection.find('img#secondpreview_1').attr("src", defult_item_image);
            newInspection.find('input[name*=item_id]').val(maxInspectionId);
            newInspection.find('span[name*=secondclose_1]').attr("value", "");
    ...
    $('#finalSubmitBtn').click(function () {
        ...
        if ($('input[name="_method"]').val() == 'POST' || $('input[name="_method"]').val() == 'PUT') {
            ...
            formData.append('inspectedSecondSignature', signaturePadinspectedSecond.toDataURL().split(',')[1]);
...
function removeImageSecond(element) {
    var itemId = element.parents('div.inspectionRow').attr('data-item-id');
    console.log(itemId)
    $('#secondpreview_' + itemId).attr('src', '../../../dist/img/preview.png');
    $('#secondclose_' + itemId).css({ 'display': 'none' });
    $('#secondimage_' + itemId).val("");
...
function saveInspectionDetails(inspectionId) {
    $('div.inspectionRow').each(function () {
        ...
        if ($('#secondimage_' + item_id)[0].files.length > 0) {
            formData.append('secondimage', $('#secondimage_' + item_id)[0].files[0]);
        }
        $.ajax({
...
function setPreview(element) {
    if (element.files && element.files[0]) {
        ...
        reader.onload = function (e) {
            $(element).closest('div.row').find('.first-preview').attr('src', e.target.result).width(48).height(48);
        };
...
function setPreviewSecond(element) {
    if (element.files && element.files[0]) {
        var reader = new FileReader();
        reader.onload = function (e) {
            $(element).closest('div.row').find('.second-preview').attr('src', e.target.result).width(48).height(48);
        };
        reader.readAsDataURL(element.files[0]);
    }
}
...
if ($('input[name="_method"]').val() == 'POST' || $('input[name="_method"]').val() == 'PUT') {
    ...
    var inspectedSecond = document.getElementById("secondinspectedSignature");
    var inspectedSecondclearButton = inspectedSecond.querySelector("[data-action=clear]");
    var inspectedSecondCanvas = inspectedSecond.querySelector("canvas");
    var signaturePadinspectedSecond;
    signaturePadinspectedSecond = new SignaturePad(inspectedSecondCanvas);
    inspectedSecondclearButton.addEventListener("click", function (event) {
        signaturePadinspectedSecond.clear();
    });
